...
public class GildedRose {
  ...
  public void updateQuality() {
    for (int i = 0; i < items.length; i++) {
      boolean normal = true;
      boolean conjured = new Conjured(items[i]).isConjured();

      if (items[i].name.equals("Aged Brie")) {
        if (conjured) {
          items[i] = new Brie(items[i]).UpdateBrie();
          items[i].sellIn += 1;
        }
        items[i] = new Brie(items[i]).UpdateBrie();
        normal = false;
      ...
      if (items[i].name.equals("Backstage passes to a TAFKAL80ETC concert")) {
        if (conjured) {
          items[i] = new Backstage(items[i]).UpdateBackstage();
          items[i].sellIn += 1;
        }
        items[i] = new Backstage(items[i]).UpdateBackstage();
        normal = false;
      }
      if (!items[i].name.equals("Sulfuras, Hand of Ragnaros") && normal) {
        if (conjured) {
          items[i] = new Normal(items[i]).UpdateNormal();
          items[i].sellIn += 1;
        }
        items[i] = new Normal(items[i]).UpdateNormal();
      }
